...
class BookingTest extends TestCase
{
    ...
    public function test_user_can_update_booking_quantity()
    {
        ...
        // First call will redirect to payment page
        $response = $this->actingAs($this->attendee)
            ->put(route('bookings.update', $booking->id), $updateData);

        // Verify we're redirected to payment page
        $response->assertRedirect(route('payment.additional', ['bookingId' => $booking->id]));

        // Check that session has the booking update data
        $this->assertEquals([
        ...
        $paymentResponse = $this->actingAs($this->attendee)
            ->post(route('payment.process.additional', $booking->id), [
                'payment_method' => 'card'
            ]);

        // Check if booking quantity is updated after payment
        $this->assertDatabaseHas('bookings', [
            'id' => $booking->id,
            'quantity' => 4,
